...
public class CreateMapCmd extends Cmd {
    ...
    public boolean run(SWCommandSender sender, String[] args) {
        ...
        CompletableFuture<? extends GameInstance> worldFuture = plugin.getGameInstanceManager().createGameWorld(template);
        worldFuture.thenAccept(gameInstance -> {
            if (gameInstance instanceof LocalGameInstance) {
                LocalGameInstance localGameInstance = (LocalGameInstance) gameInstance;
                localGameInstance.setEditing(true);
                ...
                try {
                    ...
                    templateExistsFuture = plugin.getWorldLoader().generateWorldInstance(localGameInstance);
                ...
                templateExistsFuture.thenAccept(templateExists -> {
                    ...
                    if (!templateExists) {
                        plugin.getWorldLoader().createBasePlatform(localGameInstance);
                    }

                    localGameInstance.makeReadyForEditing();

                    // Teleport the player onto the platform that was just created
                    player.teleportAsync(localGameInstance.getWorldName(),
